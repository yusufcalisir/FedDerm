"""
fedderm/federated/dp_lora_simulation.py
---------------------------------------
Federated DP-SGD + LoRA on frozen ViT-B/16 backbone simulation driver using Flower and Opacus.
"""
...
def run_federated_dp_lora(cfg: DictConfig) -> dict[str, Any]:
    """Execute DP-LoRA FedProx simulation driven by Hydra config.
    ...
    lr = float(cfg.local_training.get("lr", 1e-3))
    weight_decay = float(cfg.local_training.get("weight_decay", 1e-2))
    ...
    backbone_name = str(cfg.model.get("backbone", "vit_base_patch16_224"))
    ...
    print(f"\n[federated-dp-lora] backbone: {backbone_name} (LoRA r={lora_rank}, alpha={lora_alpha})")
    print(f"[federated-dp-lora] algorithm: DP-LoRA FedProx (mu={mu})")
    print(f"[federated-dp-lora] privacy: noise_multiplier={noise_multiplier}, max_grad_norm={max_grad_norm}, delta={target_delta}")
    print(f"[federated-dp-lora] device: {device} (cuda_available={torch.cuda.is_available()})")
    print(f"[federated-dp-lora] clients: {num_clients} | sampled/round: {clients_per_round}")
    print(f"[federated-dp-lora] rounds: {num_rounds} | local_epochs: {local_epochs}")
    print(f"[federated-dp-lora] image_size: {image_size}x{image_size} | batch_size: {batch_size}")
    print(f"[federated-dp-lora] output: {out_dir}\n")

    # -- Validate Opacus compatibility --------------------------------------------
    test_model = build_vit_lora_model(
    ...
    if compat_errors:
        print(f"[federated-dp-lora] WARNING: Opacus compatibility errors: {compat_errors}")
    else:
        print("[federated-dp-lora] Opacus ModuleValidator: PASSED (ViT-B/16 + LoRA is 100% compatible)")
    ...
    global_model = build_vit_lora_model(
        model_name=backbone_name,
        num_classes=cfg.num_classes,
        rank=lora_rank,
        lora_alpha=lora_alpha,
    ...
    def evaluate_fn(
        ...
        nonlocal best_val_macro_f1, best_val_acc
        eval_model = build_vit_lora_model(
            model_name=backbone_name,
            num_classes=cfg.num_classes,
            rank=lora_rank,
            lora_alpha=lora_alpha,
    ...
    def client_fn(context: Context) -> fl.client.Client:
        ...
        # Clients instantiate model (shares pretrained backbone structure)
    ...
    t0 = time.time()
    print("[federated-dp-lora] starting Flower DP-LoRA simulation...\n")
    ...
    # Determine GPU resources for Ray actors
    use_cuda = torch.cuda.is_available()
    gpu_count = torch.cuda.device_count() if use_cuda else 0
    client_gpus = 0.5 if use_cuda else 0.0
    ...
    if use_cuda:
        ray_init_args["num_gpus"] = gpu_count
    ...
    try:
        start_simulation(
            ...
            strategy=strategy,
            client_resources={"num_cpus": 1, "num_gpus": client_gpus},
    ...
    print(f"\n[federated-dp-lora] simulation complete in {total_time / 60:.1f} min")
    ...
    test_metrics["lora_rank"] = lora_rank
    test_metrics["lora_alpha"] = lora_alpha
    ...
    class_names = get_class_names()
    print("\n" + "=" * 55)
    print("DP-LoRA FedProx Test Evaluation Results")
    print("=" * 55)
    ...
    print(f"Server Privacy Budget:  eps = {test_metrics['server_epsilon']:.2f} (delta = {target_delta})")
    print(f"Client Privacy Budget:  eps = {test_metrics['client_epsilon']:.2f} (delta = {target_delta})")
    print("-" * 55)
    for name, f1 in zip(class_names, test_metrics["per_class_f1"]):
        print(f"  {name:<6s} F1: {f1 * 100:.2f}%")
    print("=" * 55 + "\n")
